...
class ObjectAnalyzer:
    ...
    cv2.setTrackbarPos(labLMinName, LabTrackbarName,255)
    cv2.setTrackbarPos(labLMaxName, LabTrackbarName,255)
    cv2.setTrackbarPos(labAMinName, LabTrackbarName,255)
    cv2.setTrackbarPos(labAMaxName, LabTrackbarName,255)
    cv2.setTrackbarPos(labBMinName, LabTrackbarName,255)
    cv2.setTrackbarPos(labBMaxName, LabTrackbarName,255)
    ...
    def getModifiedFrame(self, frame, circleIndex):
        ...
        PILFrame = self.getPILFrame(claheFrame)
        imshow("PILFrame", PILFrame)
        LABFrame = self.getLABFrame(PILFrame, circleIndex)
        imshow("LABFrame", LABFrame)
        redFrame = self.getRedHSVImage(LABFrame)
        imshow("redFrame",redFrame)
    ...
    def getLABFrame(self, frame, circleIndex):
        ...
        labLMin = self.labLMin
        labLMax = self.labLMax
        labAMin = self.labAMin
        labAMax = self.labAMax
        labBMin = self.labBMin
        labBMax = self.labBMax
    ...
    def normalCircleScanning(self, frame, state):
        ...
        imshow("NormalCircleScanning",frame)
        imshow("ModifiedImage",circleImage)
        values = sv.getCircleValues(state.circleReached)
        
        dp = self.houghDP
        minDist = self.houghMinDist
        param1 = self.houghParam1
        param2 =  self.houghParam2
        minRadius = self.houghMinRadius
        maxRadius = self.houghMaxRadius
